...
def _apply_mpc_grf_features(
  ...
  # Full control landmarks.  The actor remains deployment-friendly; the critic
  # can associate force/moment feasibility with the resulting imitation error.
  cfg.observations["critic"].terms["mpc_contact_force_ref"] = ObservationTermCfg(
    func=mpc_grf_mdp.mpc_contact_force_ref,
    params={"command_name": "loco_mpc"},
  )
  cfg.observations["critic"].terms["mpc_contact_moment_ref"] = ObservationTermCfg(
    func=mpc_grf_mdp.mpc_contact_moment_ref,
    params={"command_name": "loco_mpc"},
  )
...
def themis_hybrid_mimic_env_cfg(
  play: bool = False,
  motion_file: str | None = None,
  tracker_policy_path: str | None = None,
  mpc_parameter_network_path: str | None = None,
  centroidal_body_names: tuple[str, ...] | None = None,
) -> ManagerBasedRlEnvCfg:
  """HybridMimic task: frozen motion tracker plus explicit PD execution.

  ``motion_file`` must be a *THEMIS-retargeted* BeyondMimic-format NPZ.  The
  G1 files in ``/home/user/wmd/beyondmimic/data/motions`` intentionally fail
  the name check here because this repository's active robot is THEMIS.  The
  ...
  # this a joint-target correction plus a learned contact intention.
  body_names = (
    "BASE_LINK", "FEMUR_L", "TIBIA_L", "FOOT_L", "FEMUR_R", "TIBIA_R", "FOOT_R",
    "UPPERARM_L", "FOREARM_L", "LOWERWRIST_L", "UPPERARM_R", "FOREARM_R", "LOWERWRIST_R",
  )
  ...
  cfg.actions = {
    "hybrid_mimic": HybridMimicActionCfg(
      ...
      tracker_policy_path=tracker_policy_path,
      # The action implements the PD term explicitly; gains are intentionally
      # close to the position-actuator values, while motor limits remain in
      # the entity config.
      kp=STIFFNESS,
      kd=DAMPING,
    )
  ...
  for group in ("actor", "critic"):
    ...
    cfg.observations[group].terms["motion_reference_centroidal"] = ObservationTermCfg(
  ...
  # BeyondMimic's anchor/body/joint imitation objective, augmented with the
  # model-based CoM/momentum terms inherited from the current MPC framework.
...
def themis_hierarchical_hybrid_mimic_env_cfg(
  play: bool = False,
  motion_file: str | None = None,
  centroidal_body_names: tuple[str, ...] | None = None,
) -> ManagerBasedRlEnvCfg:
  """End-to-end hierarchical MPC-guided motion imitation.

  A single on-policy rollout contains two semantically separated action
  levels.  At every policy step the low level emits one whole-body joint
  action around the reference target.  Every five policy steps the
  high-level part updates bounded CD-MPC parameters (contact-clock rate,
  duty factor, touchdown Gaussian mean/std, and momentum residual).  The
  MPC remains an ordinary detached QP whose state/control landmarks enter
  the reward; neither the solver nor MuJoCo is differentiated through.

  The current rsl_rl integration has one PPO optimizer and one critic, so the
  two levels share a rollout/value baseline rather than pretending that two
  independent PPO learners are synchronized.  This is intentional: both
  action levels optimize the same imitation return, while the action term
  enforces their different control rates and physical interfaces.
  """
  cfg = themis_hybrid_mimic_env_cfg(
    play=play,
    motion_file=motion_file,
    tracker_policy_path=None,
    mpc_parameter_network_path=None,
    centroidal_body_names=centroidal_body_names,
  )
  action = cfg.actions["hybrid_mimic"]
  assert isinstance(action, HybridMimicActionCfg)
  # The BeyondMimic-style tracker is now a single low-level joint-target
  # action around the reference.  A second, continuous two-foot action is a
  # policy contact intention used to modulate the MPC schedule and supervised
  # by the simulator contact sensor.  It replaces the removed offline GRU.
  action.joint_target_residual_scale = 0.30
  action.enable_contact_state_action = True
  ...
  # Keep the high-level contact plan and QP solve aligned (50 Hz policy / 5
  # = 10 Hz).  The low-level action still applies at every policy step.
  loco_mpc.run_every_n_steps = action.high_level_decimation

  # The actor must observe the held plan it is asked to realize.  These are
  ...
  # This turns the already-computed force landmark from a diagnostic into an
  # explicit low-level objective.  CoM and momentum landmark rewards are
  # inherited from the MPC task; all whole-body imitation rewards remain in
  ...
  # Do not simultaneously force a phase-clock contact label: the policy is
...
def themis_mpc_rl_mimic_contact_env_cfg(
  play: bool = False,
  motion_file: str | None = None,
  centroidal_body_names: tuple[str, ...] | None = None,
  ...
  """Basic MPC-RL motion imitation with a fast learned contact intention.

  This is deliberately separate from the hierarchical task.  The Phase-1
  teacher policy has a 29-D whole-body reference correction and a 2N-D
  continuous contact-plan residual. The centroidal MPC receives the motion-derived CoM, linear
  momentum and angular-momentum trajectory as ``x_ref`` and keeps
  ``u_ref = 0``; its existing QP therefore implements state tracking plus
  contact-wrench magnitude/slew regularization.  MPC remains detached and
  affects PPO only through landmarks and rewards.

  The contact-plan action is transformed by a bounded residual map and frozen
  before the QP assembly. It corrects the reference-motion contact sequence
  but cannot declare physical contact by itself: the MuJoCo contact sensor
  supplies delayed plan-consistency labels.
  """
  cfg = themis_hybrid_mimic_env_cfg(
    play=play,
    motion_file=motion_file,
  ...
  motion.reference_contact_key = reference_contact_key
  motion.reference_contact_height_threshold = reference_contact_height_threshold
  motion.reference_contact_speed_threshold = reference_contact_speed_threshold
  motion.reference_contact_min_stance_frames = reference_contact_min_stance_frames
  motion.reference_contact_min_swing_frames = reference_contact_min_swing_frames
  if contact_point_offsets_b is not None:
    motion.contact_point_offsets_b = contact_point_offsets_b
  # Phase-1 tracks a finite, non-periodic reference. The terminal handling in
  # MotionReferenceCommand supplies a static tail to MPC; this term resets
  # before that tail can be mistaken for an endlessly repeated dynamic clip.
  motion.loop = False
  ...
  # Replan at the policy rate so each full-horizon contact-plan action enters
  # the next MPC call. Numerical MPC dt remains the existing fixed 0.07 s.
  loco_mpc.run_every_n_steps = 1
  action.contact_plan_horizon = loco_mpc.mpc_horizon

  # The teacher actor is causal with respect to the kinematic reference: it
  # sees one upcoming q/dq frame, never the H-frame centroidal/MPC preview.
  # The critic retains privileged landmarks for value estimation and rollout
  # storage, without exposing them at deployment.
  actor_terms = cfg.observations["actor"].terms
  for name in ("motion_reference", "motion_anchor_error", "motion_reference_centroidal"):
    actor_terms.pop(name, None)
  actor_terms["motion_reference_preview"] = ObservationTermCfg(
  ...
  cfg.terminations["motion_clip_end"] = TerminationTermCfg(
    func=mimic_mdp.motion_clip_complete,
    params={"command_name": "motion"},
  )

  # Keep exactly three reward families: reference mimic, detached MPC
  # landmarks, and regularization/safety inherited from the base task.
  ...
  cfg.rewards["future_contact_plan"] = RewardTermCfg(
    ...
    params={
      ...
      "force_on_threshold": 20.0,
      "force_off_threshold": 10.0,
    },
  )
  return cfg


def themis_mpc_rl_mimic_student_env_cfg(
  play: bool = False,
  motion_file: str | None = None,
  centroidal_body_names: tuple[str, ...] | None = None,
) -> ManagerBasedRlEnvCfg:
  """Phase-2 causal student environment: RL joint target only, no MPC term.

  The student action is a 29-D residual around the single-frame reference,
  yielding ``q_des = q_ref + Delta q`` in :class:`HybridMimicAction`. It has
  no contact-plan action and no centroidal MPC command; DAgger teacher targets
  are collected externally from the Phase-1 teacher on the same student-state
  distribution.
  """
  cfg = themis_hybrid_mimic_env_cfg(
    play=play,
  ...
  # Remove model-based command, rewards, and privileged observations. The
  # student may only use proprioception plus one upcoming q/dq reference.
  ...
  for group in ("actor", "critic"):
    ...
    for name in list(terms):
      if name.startswith("mpc_") or name == "policy_contact_state":
        terms.pop(name)
  actor_terms = cfg.observations["actor"].terms
  for name in ("motion_reference", "motion_anchor_error", "motion_reference_centroidal"):
    actor_terms.pop(name, None)
...
def themis_motion_tracker_env_cfg(
  play: bool = False,
  motion_file: str | None = None,
) -> ManagerBasedRlEnvCfg:
  """Stage-one BeyondMimic-style tracker used before residual-policy training.

  Train this task first, export the policy to TorchScript, then provide that
  export as ``tracker_policy_path`` in :func:`themis_hybrid_mimic_env_cfg`.
