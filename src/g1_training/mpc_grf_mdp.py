"""Paper-compatible G1 centroidal-MPC velocity and loco-manipulation MDP."""
...
from g1_mpc.centroidal_mpc import CentroidalMPC, MPCConfig, MPCInput, MPCModelParameters
from g1_mpc.contact_schedule import make_walking_schedule
...
class LocoMPCCommandCfg(CommandTermCfg):
    ...
    mpc_horizon: int = 10
    mass: float = 37.0
    inertia_body: tuple[tuple[float, float, float], ...] = G1_CENTROIDAL_INERTIA_BODY
    hip_width: float = 0.1
    foot_x_toe: float = G1_MPC_FOOT_X_TOE
    foot_x_heel: float = G1_MPC_FOOT_X_HEEL
    foot_y_half: float = G1_MPC_FOOT_Y_HALF
    mu_foot: float = G1_MPC_MU_FOOT
    mu_foot_yaw: float = G1_MPC_MU_FOOT_YAW
    fz_max_foot: float = G1_MPC_FZ_MAX_FOOT
    # These resolve the exact simulator geoms whose randomized friction is
    # copied into the per-environment centroidal QP at reset.
    ...
    def _sync_mpc_model_parameters(self, env_ids: Tensor) -> None:
        ...
        for foot, geom_ids in enumerate((self._left_foot_geom_model_ids, self._right_foot_geom_model_ids)):
            if geom_friction is not None and geom_ids.numel() > 0:
                # The least-friction collision geom is the conservative QP cone.
