...
def make_reference_contact_schedule(
    ...
    """Build the fixed-horizon schedule used by the Jingchu01 mimic teacher.

    The reference contact sequence and positions are the nominal schedule.
    The optional action is only a bounded *activation* residual and is frozen
    before QP assembly.  Learned timing/foothold distributions deliberately
    live in :mod:`jingchu01_mpc.experimental_contact_schedule`, not here.
    """
    state = reference_contact_state.to(device=device, dtype=dtype)
    r_lf = reference_r_LF.to(device=device, dtype=dtype)
    r_rf = reference_r_RF.to(device=device, dtype=dtype)
    if state.shape != (B, N, 2) or r_lf.shape != (B, N, 3) or r_rf.shape != (B, N, 3):
        raise ValueError("reference contact state/positions must be [B,N,2], [B,N,3], [B,N,3]")
    sigma = torch.zeros(B, N, 4, device=device, dtype=dtype)
    sigma[:, :, :2] = state.clamp(0.0, 1.0)
    if policy_contact_plan_residual is not None:
        residual = policy_contact_plan_residual.to(device=device, dtype=dtype)
        if residual.shape != (B, N, 2):
            raise ValueError("policy_contact_plan_residual must have shape [B,N,2]")
        if not 0.0 <= policy_contact_plan_residual_scale <= 1.0:
            raise ValueError("policy_contact_plan_residual_scale must lie in [0,1]")
        sigma[:, :, :2] = (sigma[:, :, :2] + policy_contact_plan_residual_scale * torch.tanh(residual)).clamp(0.0, 1.0)
    if preserve_nominal_support:
        nominal_support = state.amax(dim=-1) >= 0.5
        restore = nominal_support & (sigma[:, :, :2].amax(dim=-1) < 0.5)
        restored = sigma[:, :, :2].scatter(-1, state.argmax(dim=-1, keepdim=True), 0.5)
        sigma[:, :, :2] = torch.where(restore.unsqueeze(-1), restored, sigma[:, :, :2])

    def _expand_rot(rotation: Tensor | None) -> Tensor | None:
        if rotation is None:
            return None
        rotation = rotation.to(device=device, dtype=dtype)
        return rotation.unsqueeze(0).expand(B, -1, -1).contiguous() if rotation.dim() == 2 else rotation.contiguous()

    return ContactSchedule(sigma=sigma, r_LF=r_lf, r_RF=r_rf,
                           r_LH=torch.zeros(B, N, 3, device=device, dtype=dtype),
                           r_RH=torch.zeros(B, N, 3, device=device, dtype=dtype),
                           R_LF=_expand_rot(R_LF_rot), R_RF=_expand_rot(R_RF_rot))
