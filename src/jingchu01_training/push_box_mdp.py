...
def box_com_tracking(
    ...
    # Robot-local MPC packages intentionally define independent command
    # classes.  The landmark contract is structural: a loco-manipulation term
    # exposes a horizon, dt and the planned box trajectory.  Requiring the
    # THEMIS concrete type here would silently disable this reward for G1/JC01.
    if term is None or not all(hasattr(term, name) for name in ("_vis_box_ref_traj", "cfg")):
        return torch.zeros(env.num_envs, device=env.device)
